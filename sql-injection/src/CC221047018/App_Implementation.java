...
public class App_Implementation {
	...
	public void add_inStatement() throws IOException, ClassNotFoundException {
		try {
			...
			String query = "select count(*) from college_administrators where userID = '"+ userID +"' and password = '"+password+"'";
			...
			if(count == 1) {
				...
				String pdetails = "Select * from students_details";
				ResultSet rs = db.query_normalStatement(pdetails);
				
				while (rs.next()) {
					System.out.println(rs.getInt("s_id")+"   "+rs.getString("s_name")+"   "+rs.getString("s_branch"));
	            }
			}else {
	...
	public void add_inPreStatement() throws IOException, ClassNotFoundException {
		try {
			...
			String query = "select count(*) from college_administrators where userID = ?  and password = ?;";
			...
			if(count == 1) {
				...
				String sDetails = "insert into  students_details values (?,?,?);";
